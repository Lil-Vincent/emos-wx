<template>
  <view class="page">
    <view class="list">
      ...
      <view class="item">
        ...
        <picker
          ...
          :range="statusArray"
          @change="statusChange"
          >{{ statusArray[statusIndex] }}</picker
        >
        <text v-if="!canEdit" class="value">{{
      ...
      <scroll-view class="dialog" scroll-y="true">
        ...
        <checkbox-group @change="roleChange">
          <label class="list-item" v-for="one in roleArray" :key="one.id">
            <checkbox
              :value="one.id"
              :checked="roles.split('，').indexOf(one.roleName) != -1"
            />
    ...
    <button
      ...
      v-if="
        opt != 'read' &&
        checkPermission(['ROOT', 'EMPLOYEE:INSERT', 'EMPLOYEE:UPDATE'])
      "
      @tap="save"
    >
      保存
    ...
    <button
      class="delete-btn"
      v-if="opt != 'read' && checkPermission(['ROOT', 'EMPLOYEE:DELETE'])"
      @tap="deleteUser"
    >
      删除
    </button>
